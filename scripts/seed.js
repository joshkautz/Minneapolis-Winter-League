...
async function seedTestData() {
	...
	try {
		...
		} else if (config.mode === 'setup-only') {
			// Setup foundation only (seasons, teams, players)
			await seedSetupOnly()
		} else {
			// Full seeding
			await seedFullData()
...
/**
 * Setup only mode - creates foundation data (seasons, teams, players) without games
 */
async function seedSetupOnly() {
	console.log('🎯 Setting up foundation data only (seasons, teams, players)...')
	...
	// Create teams for historical and current seasons
	const teams = await createTeamsForActiveSeasons(pastSeasons, players)
	...
	console.log('\n🎯 Foundation setup complete! Ready for weekly game seeding.')
}
	...
	if (!isPlayoff) {
		...
		for (let roundIndex = 0; roundIndex < weekSchedule.length; roundIndex++) {
			...
			for (let fieldIndex = 0; fieldIndex < round.length; fieldIndex++) {
				...
				let gameData
				if ((isHistorical && !onlyFutureGames) || config.mode === 'weekly') {
					const scores = generateGameScores()
					gameData = {
						away: createRef(Collections.TEAMS, awayTeam.id),
						awayScore: scores.awayScore,
...
function getGameDates(startDate, endDate) {
	...
	// Collect Saturdays until we exceed the end date or have 7 dates
	// Compare dates only (ignore time) to handle timezone issues
	while (saturdays.length < 7) {
		// Check if current date is on or before end date (ignoring time)
