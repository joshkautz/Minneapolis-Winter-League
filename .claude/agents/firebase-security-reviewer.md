...
Function is the *only* thing standing between a request and the database. A
...
- Admin-only operations must call `validateAdmin`. Admin comes from the player
  document's `admin` boolean, not Auth custom claims — a function that trusts
  `auth.token` for admin is wrong.
- Does the function confirm the caller *owns* the resource, not merely that
