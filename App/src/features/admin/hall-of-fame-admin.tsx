/**
 * Admin interface for Hall of Fame calculations
 *
...
export const HallOfFameAdmin: React.FC = () => {
	...
	const handleTriggerCalculation = async (type: 'full' | 'incremental') => {
		...
		try {
			const result = await triggerHallOfFameCalculation({
				calculationType: type,
				applyDecay: true,
			})

			setCalculationSuccess(
				`${type} calculation started successfully! ID: ${result.data.calculationId}`
			)
		} catch (err) {
			setCalculationError(
			...
			<Card>
				...
				<CardContent className='space-y-4'>
					<p className='text-sm text-muted-foreground'>
						Choose the type of calculation to perform. Full calculations rebuild
						all rankings from scratch, while incremental calculations continue
